    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm">{unique.filter(a => !a.is_read).length} unread</p>
        </div>
      ...
      <div className="flex gap-2">
        {['', 'critical', 'warning', 'info'].map(s => (
          <button key={s} onClick={() => setFilter(s)}
            className={`px-3 py-1.5 rounded-lg text-sm capitalize transition-all ${filter === s ? 'bg-primary-600 text-white' : 'bg-surface-700 text-gray-400'}`}>
            {s || 'All'}
          </button>
        ))}
      </div>
      <div className="space-y-2">
        {loading ? <div className="flex justify-center py-10"><RefreshCw size={20} className="animate-spin text-primary-400" /></div> :
          unique.length === 0 ? <div className="card text-center py-12 text-gray-500">No alerts to display</div> :
          unique.map((alert, i) => (
