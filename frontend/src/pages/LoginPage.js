...
export default function LoginPage() {
  ...
  const handleGoogleMock = async () => {
    setLoading(true);
    try {
      const user = await login('citizen@disaster.com', 'Cit@123');
      ...
      <div className="w-full md:w-1/2 min-h-screen md:min-h-0 flex items-center justify-center p-6 lg:p-12 relative z-20">
        <div className="w-full max-w-[420px] z-10 block mb-12 md:mb-0">
          ...
          <div className="bg-white/80 dark:bg-white/5 backdrop-blur-xl border border-surface-200 dark:border-white/10 shadow-lg dark:shadow-none hover:scale-[1.02] transition-all duration-300 rounded-2xl p-8 relative">
            ...
            <button type="button" onClick={handleGoogleMock} disabled={loading} className="w-full bg-surface-50 dark:bg-white/5 border border-surface-200 dark:border-white/10 hover:bg-surface-100 dark:hover:bg-white/10 text-surface-700 dark:text-gray-300 font-medium py-3 rounded-xl transition-all duration-300 flex items-center justify-center gap-3 active:scale-[0.98] text-sm">
