    ...
    <div className="min-h-screen bg-surface-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">Join DisasterWatch</h1>
          <p className="text-gray-400 mt-1">Help your community be prepared</p>
        </div>
        <div className="card">
              ...
              <div className="grid grid-cols-3 gap-2">
                {['citizen', 'volunteer'].map(role => (
                  <button key={role} type="button" onClick={() => setForm(f => ({ ...f, role }))}
                    className={`py-2 rounded-lg text-sm font-medium capitalize border transition-all ${form.role === role ? 'bg-primary-600 border-primary-500 text-white' : 'bg-surface-700 border-surface-600 text-gray-400 hover:text-white'}`}>
                    {role === 'citizen' ? '👤' : '🦺'} {role}
                  </button>
                ))}
              </div>
