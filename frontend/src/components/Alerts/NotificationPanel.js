...
export default function NotificationPanel({ onClose }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-80 bg-surface-800 border-l border-surface-700 shadow-2xl z-20 flex flex-col animate-slide-in">
      <div className="flex items-center justify-between p-4 border-b border-surface-700">
        ...
        <div className="flex items-center gap-2">
          <button onClick={markAllRead} className="text-gray-400 hover:text-white text-xs transition-colors">Mark all read</button>
          <button onClick={onClose} className="text-gray-400 hover:text-white p-1 rounded-lg hover:bg-surface-700 transition-colors">
            <X size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {all.length === 0 ? (
          ...
          all.map((item, i) => (
            <div key={i} className="flex items-start gap-3 p-4 border-b border-surface-700 hover:bg-surface-700 transition-colors cursor-pointer">
